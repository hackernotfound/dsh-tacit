...
function capScopes(list, seenAt) {
  const live = list.filter((entry) => entry.status !== 'retired' && scopeOf(entry) !== '')
  ...
  return list.filter((entry) => entry.status === 'retired' || entry.source === 'user' || !dropped.has(scopeOf(entry)))
}

/** At most MAX_DIRECTIVES global and MAX_WORKSPACE_DIRECTIVES per-workspace live directives over at most MAX_SCOPES workspaces, plus the last MAX_RETIRED retired ones; order kept. */
