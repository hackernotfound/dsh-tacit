 ...
 */
export function clipDirective(value, max = DIRECTIVE_MAX_CHARS, min = 100) {
...
export const DIRECTIVE_TOOL = {
  ...
  parameters: {
    type: 'object',
    properties: {
      directives: { type: 'array', items: { type: 'string', maxLength: 220, description: 'one sentence, at most 25 words' }, minItems: 1, maxItems: 4 },
    },
    required: ['directives'],
  },
}
...
export function buildDirectiveUserText(profile, recentReports = []) {
  ...
  if (existing.length > 0) {
    ...
    for (const entry of existing) lines.push('- ' + clipDirective(entry.text))
  }
...
export function classifyDirectives(text) {
  ...
  for (const item of raw) {
    ...
    const value = clipDirective(item)
 ...
 */
export function buildSteeringSection(profile) {
...
export function renderSteeringSection(profile) {
  return buildSteeringSection(profile).text
}
