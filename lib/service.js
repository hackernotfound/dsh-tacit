...
export function createCoachService(ctx, store, effectiveConfig) {
  ...
  const closeRun = (runId, results = {}) => usage.endRun(runId, { results }) ?? usage.runSummary(runId)
  ...
  const runAnalysis = (sessionId, turn, { trigger = 'manual', followUp = '', digest = null, previousDigest = null, runId = '' } = {}) => {
    ...
    return exclusive.then((result) => ({ ...result, run: usageRunId === '' ? null : usage.runSummary(usageRunId) }))
  }
  ...
  const bootstrapState = { running: false, done: 0, total: 0, startedAt: 0, runId: '', billedCalls: 0, unpricedCalls: 0, usdKnown: 0, tokens: 0 }
  ...
  const refreshBootstrapUsage = () => {
    ...
    bootstrapState.tokens = totalTokens(summary.tokens)
  }
  ...
  const runBootstrap = async ({ sessionId, limit }) => {
    ...
    if (typeof sessionId === 'string' && sessionId.length > 0) {
      const { session, turns } = turnsOf(svc, sessionId)
      if (session === undefined) return { ok: false, analyzed: 0, skipped: 0, directives: 0, code: 'no-session', detail: '', run: null }
