...
export function createCoachService(ctx, store, effectiveConfig) {
  ...
  const mergeDirectives = (profile, items) => {
    ...
    for (const item of items) {
      ...
      // A new distilled directive goes on trial against the current correction and messy-turn rates.
      if (!baselines.has(scope)) baselines.set(scope, baselinesFor(scope))
      distilled.push({
        id: nextDirectiveId(),
        text: item.text,
        enabled: true,
        ...
        createdAt: Date.now(),
        status: 'candidate',
        trial: { turns: 0, messy: 0, corrected: 0, ...baselines.get(scope), startedAt: Date.now() },
