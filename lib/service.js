...
export const MAX_REWRITE_RECORDS = 50
/** Pending outcome verifications kept per session (FIFO, oldest dropped). */
export const MAX_PENDING_VERIFICATIONS = 20
...
export function mergeConfig(base, patch) {
  ...
  merged.learningThreshold = Math.max(1, Math.min(100000, Math.round(Number(merged.learningThreshold) || 20)))
...
export function sessionLabelOf(service, sessionId) {
  const session = typeof service.sessions?.get === 'function' ? service.sessions.get(sessionId) : undefined
...
export function recentContextOf(turns) {
...
export function outcomeScore(turn) {
...
export function outcomeBaselineOf(turn) {
...
export function lastFinishedTurnOf(turns) {
...
export function createCoachService(ctx, store, effectiveConfig) {
  ...
  const safeProfile = () => {
    ...
    return parsed.success
      ? parsed.data
      : { analyzedCount: 0, patterns: [], updatedAt: 0, styleRules: [], goodExamples: [], feedbackLog: [], pendingDistill: 0, directives: [], analysesSinceDirectives: 0 }
  }
  let directiveSeq = 0
  ...
  const capAndSaveProfile = (profile) => {
    ...
    profile.goodExamples = profile.goodExamples.slice(-MAX_GOOD_EXAMPLES)
  ...
  const service = {
    ...
    async improveDraft(args) {
      ...
      if (!parsed.success) {
        return { ok: false, improved: '', rationale: '', savingsEstimate: 0, rewriteId: '', patternsUsed: [], code: 'bad-request', detail: '' }
      }
      const { sessionId, draft } = parsed.data
      const config = effectiveConfig()
      const profile = safeProfile()
      // No learning gate: whatever the profile has learned so far rides along.
      const ready = true
      const svc = serviceOf(ctx)
      ...
      // Only trusted (or still-inexperienced) patterns reach the prompt, and
      // only past the learning threshold; style rules + the last 3 verbatim
      // down-reasons ride along for free on this existing call.
      const selected = ready ? improvePatterns(profile, config.maxPatterns) : []
      const userText = buildImproveUserText({
        draft: draft.trim(),
        profile: ready ? { patterns: selected } : { patterns: [] },
        recentContext,
        styleRules: ready ? profile.styleRules : [],
        negativeFeedback: ready ? lastDownReasons(profile, 3) : [],
      })
      try {
        ...
        if (text.trim() === '') {
          return { ok: false, improved: '', rationale: '', savingsEstimate: 0, rewriteId: '', patternsUsed: [], code: 'empty-response', detail: '' }
        }
        let parsed = parseJsonObject(text)
        if (parsed === null) {
          const repaired = await callCoachModel(ctx, {
        ...
        const detail = error instanceof Error ? error.message.slice(0, 300) : String(error).slice(0, 300)
        return { ok: false, improved: '', rationale: '', savingsEstimate: 0, rewriteId: '', patternsUsed: [], code: coachErrorCode(error), detail }
      }
    },
    ...
    async feedback(args) {
      ...
      if (verdict === 'up') {
        ...
        profile.goodExamples = [...profile.goodExamples, {
          prompt: record.draft,
