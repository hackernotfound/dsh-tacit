 ...
 *
 * Every captured text passes through `redactSecrets`. This fold is the only
 * path from the session log into Tacit, and the digest is what reports,
 * corrections, recent context and every model call read.
 *
...
export function applyTimeline(state, event, bounds) {
  ...
  switch (event.type) {
    ...
    case 'user/message': {
      ...
      const text = data !== null && typeof data === 'object' ? redactSecrets(textOfBlocks(data.content)) : ''
      ...
      if (current.provisionalPrompt === '') current.provisionalPrompt = clip(text, bounds.maxPromptChars)
    ...
    case 'tool/call': {
      ...
      state.current.toolCalls.push({
        ...
        args: clip(redactSecrets(data.arguments), bounds.maxToolCallChars),
      })
      return { ...state }
    }
    ...
    case 'assistant/message': {
      ...
      const text = data.message !== null && typeof data.message === 'object'
        ? redactSecrets(textOfBlocks(data.message.content))
        : ''
...
export function createTimelineDefinition(boundsOf) {
  ...
  const definition = {
    ...
    // 3 → 4: captured text is masked by `redactSecrets` before it is stored.
    stateVersion: 4,
  }
  return definition
}
