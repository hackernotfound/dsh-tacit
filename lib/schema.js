...
export const Config = z.preprocess((v) => v ?? {}, z.object({
  ...
  /** Opt-in: a distilled directive stays queued until *Start trial* is pressed on it. */
  reviewCandidates: z.boolean().default(false),
}))
...
const configPatchSchema = z.object({
  ...
  reviewCandidates: z.boolean().optional(),
})
...
/** One analysis report a distillation read: ids only, never the prompt. */
...
/**
 * queued = waiting for its scope's trial slot (or, under `reviewCandidates`, for the user's *Start trial*);
 * candidate = injected on trial; active = proven (or user-made); retired = made things worse;
 * removed = deleted by the user. Retired and removed records are kept so the distiller is told
 * not to propose them again.
 */
export const DIRECTIVE_STATUSES = ['queued', 'candidate', 'active', 'retired', 'removed']

const directiveSchema = z.preprocess(
  (raw) => {
    if (raw === null || typeof raw !== 'object') return raw
    const entry = { ...raw }
    // Profiles written before provenance was recorded: the text is as old as the record.
    ...
    if (entry.status === 'candidate' && entry.enabled === false) {
      entry.status = 'queued'
      delete entry.trial
    }
    return entry
  ...
  z.object({
    ...
    createdAt: z.number(),
    /** Last time the text changed (a reword) or the record moved state. */
    updatedAt: z.number(),
    /** Bumped on every reword of the text. */
    version: z.number().int().min(1).default(1),
    status: z.enum(DIRECTIVE_STATUSES).default('active'),
    ...
    /** The reports the distillation that wrote this text read (newest first, capped). */
    evidence: z.array(directiveEvidenceSchema).default([]),
    /** The usage-ledger run of that distillation, so its cost is one lookup away. '' for user-typed. */
    distillationRunId: z.string().default(''),
    /** When the trial reached its verdict; 0 = never. */
    evaluatedAt: z.number().default(0),
    /** When the user pressed *Start trial* under `reviewCandidates`; 0 = not yet. */
    approvedAt: z.number().default(0),
  }),
)
...
export const profileSchema = z.object({
  ...
  /** When a session was last loaded in each directive's workspace, so the least recently seen one is dropped first. */
  workspaceSeenAt: z.record(z.string(), z.number()).default({}),
})
...
export const directivesArgSchema = z.discriminatedUnion('action', [
  ...
  z.object({ action: z.literal('rescope'), id: z.string().min(1).max(64), workspace: z.string().max(1000) }),
  z.object({ action: z.literal('remove'), id: z.string().min(1).max(64) }),
  z.object({ action: z.literal('start-trial'), id: z.string().min(1).max(64) }),
])

/** `/api/tacit/directive-receipt`: one directive's provenance and cost, by id. */
export const directiveReceiptArgSchema = z.object({
...
export const analyzeArgSchema = z.object({
  ...
  /** Re-analyze a turn that already has a report (otherwise `already-analyzed`, at no cost). */
...
export const usageSummarySchema = z.object({
  ...
  byProvider: z.record(usageTotalsSchema).default({}),
  byTrigger: z.record(usageTotalsSchema).default({}),
